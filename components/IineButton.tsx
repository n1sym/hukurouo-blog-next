...
export function IineButton({webhook_url, title}:{webhook_url: string, title: string}) {
  ...
  return (
    <>
      <button className="bg-transparent text-blue-700 border py-2 px-4 ml-4 mr-2 rounded-full hover:bg-gray-100" disabled={isDisplay ? true : false } onClick={() => postIine(title)}>
        👍
      </button>
